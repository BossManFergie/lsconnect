{if !$standalone}<li>{/if}
    ...
    <div class="post {if $boosted}boosted{/if}" data-id="{$post['post_id']}">
        ...
        <div class="post-body">
            ...
            <div class="post-header">
                ...
                <div class="post-meta">
                    ...
                    <div>
                        {if $user->_logged_in}
                            ...
                            <div class="pull-right flip dropdown">
                                ...
                                <ul class="dropdown-menu post-dropdown-menu">
                                    {if $post['manage_post'] && $post['post_type'] == "product"}
                                        <li>
                                    ...
                                    {if $post['manage_post']}
                                        ...
                                        {if $post['post_type'] == "product"}
                                            <li>
                                                <a href="" data-toggle="modal" data-url="posts/product_editor.php?post_id={$post['post_id']}">
                                                    <div class="action no-desc">
                            ...
                            {if $post['post_type'] == "shared"}
                                ...
                                <a href="{$system['system_url']}/posts/{$post['origin']['post_id']}">
                                    ...
                                    {elseif $post['origin']['post_type'] == 'link'}
                                        {__("link")}
                                    {elseif $post['origin']['post_type'] == 'poll'}
                                        {__("poll")}
                        ...
                        <!-- post feeling -->
                        {if $post['feeling_action']}
                        <span class="post-title">
                            {__("is")} <i class="twa twa-lg twa-{$post['feeling_icon']}"></i> {__($post["feeling_action"])} {__($post["feeling_value"])}
                        </span>
                        {/if}
                        <!-- post feeling -->
                    </div>
            ...
            <!-- product -->
            {if $post['post_type'] == "product" && $post['product']}
                {if $post['product']['available']}
                ...
                {else}
                    <div class="mb5 text-lg x-muted">
                        <strong>({__("SOLD")}) {$post['product']['name']}</strong>
                    </div>
                {/if}
                <div class="mb5 text-bg">
                    <i class="fa fa-money fa-fw"></i> <span class="green">{$system['system_currency_symbol']}{$post['product']['price']}</span> ({$system['system_currency']})
                </div>
                {if $post['product']['location']}
                    <div class="mb10 text-muted">
                        <i class="fa fa-map-marker fa-fw"></i> {$post['product']['location']}
                    </div>
                {/if}
            {/if}
            <!-- product -->
            ...
            {if $post['post_type'] == "album" || ($post['post_type'] == "product" && $post['photos_num'] > 0) || $post['post_type'] == "photos" || $post['post_type'] == "profile_picture" || $post['post_type'] == "profile_cover" || $post['post_type'] == "page_picture" || $post['post_type'] == "page_cover" || $post['post_type'] == "group_picture" || $post['post_type'] == "group_cover"}
                <div class="mt10 clearfix">
                    <div class="pg_wrapper">
                        {if $post['photos_num'] == 1}
                            <div class="pg_1x">
                                <a href="{$system['system_url']}/photos/{$post['photos'][0]['photo_id']}" class="js_lightbox" data-id="{$post['photos'][0]['photo_id']}" data-image="{$system['system_uploads']}/{$post['photos'][0]['source']}" data-context="{if $post['post_type'] == 'product'}post{else}album{/if}">
            ...
            {elseif $post['post_type'] == "poll" && $post['poll']}
                <div class="poll-options mt10" data-poll-votes="{$post['poll']['votes']}">
                    {foreach $post['poll']['options'] as $option}
                        <div class="mb5">
                            <div class="poll-option js_poll-vote" data-id="{$option['option_id']}" data-option-votes="{$option['votes']}">
                                <div class="percentage-bg" {if $post['poll']['votes'] > 0} style="width: {($option['votes']/$post['poll']['votes'])*100}%"{/if}></div>
                                <div class="radio radio-primary radio-inline">
                                    <input type="radio" name="poll_{$post['poll']['poll_id']}" id="option_{$option['option_id']}" {if $option['checked']}checked{/if}>
            ...
            <!-- product -->
            {if $post['post_type'] == "product" && $post['product'] && $post['author_id'] != $user->_data['user_id'] }
                <div class="mt10 clearfix">
                    <button type="button" class="btn btn-primary btn-sm pull-right flip js_chat-start" data-uid="{$post['author_id']}" data-name="{$post['post_author_name']}"><i class="fa fa-comments-o"></i> {__("Contact Seller")}</button>
                </div>
            {/if}
            <!-- product -->
