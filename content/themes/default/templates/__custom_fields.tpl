	...
	{foreach $_custom_fields as $custom_field}
        <div class="form-group">
            {if $custom_field['type'] == "textbox"}
                <input name="fld_{$custom_field['field_id']}" type="text" class="form-control" placeholder="{$custom_field['label']}">
            {elseif $custom_field['type'] == "textarea"}
                <textarea name="fld_{$custom_field['field_id']}" class="form-control" placeholder="{$custom_field['label']}"></textarea>
            {elseif $custom_field['type'] == "selectbox"}
                ...
                <select name="fld_{$custom_field['field_id']}" class="form-control">
                	<option selected value="none">{__("Select")} {$custom_field['label']}</option>
	...
	{foreach $_custom_fields as $custom_field}
	    <div class="form-group">
	        ...
	        <div class="col-sm-9">
	            ...
	            {elseif $custom_field['type'] == "selectbox"}
	                <select name="fld_{$custom_field['field_id']}" class="form-control">
	                    <option {if $custom_field['value'] == ""}selected{/if} value="none">{__("Select")} {$custom_field['label']}</option>
	                    {foreach $custom_field['options'] as $id => $value}
