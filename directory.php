...
try {
	...
	switch ($_GET['view']) {
		...
		case 'groups':
			...
			require('includes/class-pager.php');
			$params['selected_page'] = ( (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
			$total = $db->query("SELECT * FROM groups") or _error(SQL_ERROR);
			$params['total_items'] = $total->num_rows;
			$params['items_per_page'] = $system['max_results'];
			...
			// get groups
			$rows = array();
			$get_rows = $db->query("SELECT * FROM groups ".$limit_query) or _error(SQL_ERROR);
			while($row = $get_rows->fetch_assoc()) {
				$row['group_picture'] = $user->get_picture($row['group_picture'], 'group');
                /* check if the viewer joined the group */
                $row['i_joined'] = false;
                if($user->_logged_in) {
                    $check_membership = $db->query(sprintf("SELECT * FROM groups_members WHERE group_id = %s AND user_id = %s", secure($row['group_id'], 'int'), secure($user->_data['user_id'], 'int') )) or _error(SQL_ERROR);
