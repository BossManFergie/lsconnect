...
api['pages_groups/delete']  = ajax_path+"pages_groups/delete.php";
...
$(function() {
    ...
    $('body').on('click', '.js_verify, .js_unverify', function () {
        ...
        $.post(api['users/verify'], data , function(response) {
            if(response.callback) {
                ...
                if(_do == 'request') {
                    _this.after('<div class="btn btn-warning btn-friends js_unverify" '+html+'><i class="fa fa-clock-o mr5"></i>'+__["Pending"]+'</div>');
                } else {
    ...
    $('body').on('click', '.js_boost-page, .js_unboost-page', function () {
        var _this = $(this);
        var id = _this.data('id');
        var _do = (_this.hasClass('js_boost-page'))? 'boost' : 'unboost';
        /* show loading */
        _this.button('loading');
        /* post the request */
        $.post(api['users/connect'], {'do': _do, 'id': id} , function(response) {
    ...
    $('body').on('click', '.js_delete-page, .js_delete-group', function (e) {
        ...
        confirm(__['Delete'], __['Are you sure you want to delete this?'], function() {
            $.post(api['pages_groups/delete'], {'handle': handle, 'id': id} , function(response) {
                /* check the response */
                if(response.callback) {
                    eval(response.callback);
                } else {
