...
class User {
    ...
    public function __construct() {
        ...
        if(isset($_COOKIE[$this->_cookie_user_id]) && isset($_COOKIE[$this->_cookie_user_token])) {
            ...
            if($query->num_rows > 0) {
                ...
                /* get pages */
                $this->_data['pages'] = $this->get_pages();
                /* get groups */
                $this->_data['groups'] = $this->get_groups();
                /* get package */
    ...
    public function search_quick($query) {
        ...
        $results = array();
        /* search users */
        $get_users = $db->query(sprintf('SELECT user_id, user_name, user_fullname, user_gender, user_picture, user_subscribed, user_verified FROM users WHERE user_name LIKE %1$s OR user_fullname LIKE %1$s LIMIT %2$s', secure($query, 'search'), secure($system['min_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        if($get_users->num_rows > 0) {
            while($user = $get_users->fetch_assoc()) {
                $user['user_picture'] = $this->get_picture($user['user_picture'], $user['user_gender']);
                /* get the connection between the viewer & the target */
    ...
    public function publisher(array $args = array()) {
        ...
        $post['text'] = parse($post['text_plain']);
    ...
    public function get_posts(array $args = array()) {
        ...
        $valid['filter'] = array('all', '', 'photos', 'video', 'audio', 'file', 'poll', 'product', 'map');
    ...
    public function who_votes($option_id, $offset = 0) {
        ...
        $voters = array();
        $offset *= $system['max_results'];
        $get_voters = $db->query(sprintf("SELECT users.user_id, users.user_name, users.user_fullname, users.user_picture, users.user_gender FROM users_polls_options INNER JOIN users ON users_polls_options.user_id = users.user_id WHERE option_id = %s LIMIT %s, %s", secure($option_id, 'int'), secure($offset, 'int', false), secure($system['max_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
    ...
    public function comment($handle, $node_id, $message, $photo) {
        ...
        /* parse text */
        $comment['text_plain'] = $comment['text'];
        $comment['text'] = parse($comment['text_plain']);
    ...
    public function edit_privacy($post_id, $privacy) {
        ...
        if($post['manage_post'] && $post['user_type'] == 'user' && !$post['in_group'] && $post['post_type'] != "product") {
            /* update privacy */
    ...
    public function add_vote($option_id) {
        ...
        if($vote) {
            ...
            /* post notification */
            $this->post_notification($post['author_id'], 'vote', 'post', $post['post_id']);
        }
    }
    ...
    public function page_edit($page_id, $category, $title, $username_new, $username_old, $description) {
        ...
        if(strtolower($username_new) != strtolower($username_old)) {
            ...
            if(reserved_username($username_new)) {
                throw new Exception(__("You can't use")." <strong>".$username_new."</strong> ".__("as username"));
            }
    ...
    public function group_create($title, $username, $description) {
        ...
        /* insert new group */
        $db->query(sprintf("INSERT INTO groups (group_admin, group_name, group_title, group_description, group_date) VALUES (%s, %s, %s, %s, %s)", secure($this->_data['user_id'], 'int'), secure($username), secure($title), secure($description), secure($date) )) or _error(SQL_ERROR_THROWEN);
    }
    ...
    public function get_affiliates($user_id, $offset = 0) {
        ...
        $affiliates = array();
        $offset *= $system['max_results'];
        $get_affiliates = $db->query(sprintf('SELECT user_id, user_name, user_fullname, user_gender, user_picture FROM users WHERE user_referrer_id = %s LIMIT %s, %s', secure($user_id, 'int'), secure($offset, 'int', false), secure($system['max_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
    ...
    public function settings($edit, $args) {
        ...
        switch ($edit) {
            ...
            case 'basic':
                ...
                /* update user */
                $db->query(sprintf("UPDATE users SET user_fullname = %s, user_gender = %s, user_birthdate = %s, user_relationship = %s, user_biography = %s, user_website = %s WHERE user_id = %s", secure($args['fullname']), secure($args['gender']), secure($args['birth_date']), secure($args['relationship']), secure($args['biography']), secure($args['website']), secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
                break;
    ...
    public function sign_up($args) {
        ...
        /* check IP */
        $this->_check_ip();
        if(is_empty($args['full_name']) || is_empty($args['username']) || is_empty($args['password'])) {
            throw new Exception(__("You must fill in all of the fields"));
        }
        if(!valid_username($args['username'])) {
            throw new Exception(__("Please enter a valid username (a-z0-9_.) with minimum 3 characters long"));
        ...
        /* register user */
        $db->query(sprintf("INSERT INTO users (user_name, user_email, user_phone, user_password, user_fullname, user_gender, user_birthdate, user_registered, user_activation_key) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", secure($args['username']), secure($args['email']), secure($args['phone']), secure(md5($args['password'])), secure(ucwords($args['full_name'])), secure($args['gender']), secure($args['birth_date']), secure($date), secure($activation_key) )) or _error(SQL_ERROR_THROWEN);
        /* get user_id */
        ...
        if($system['activation_enabled']) {
            if($system['activation_type'] == "email") {
                /* prepare activation email */
                $subject = __("Just one more step to get started on")." ".$system['system_title'];
                $body  = __("Hi")." ".ucwords($args['full_name']).",";
                $body .= "\r\n\r\n".__("To complete the sign-up process, please follow this link:");
    ...
    public function sign_in($username, $password, $remember = false) {
        ...
        if(!valid_username($username)) {
            throw new Exception(__("Please enter a valid username"));
        }
        if(!$this->check_username($username)) {
            throw new Exception(__("The username you entered does not belong to any account"));
        }
        /* get user */
        $get_user = $db->query(sprintf("SELECT user_id FROM users WHERE user_name = %s AND user_password = %s", secure($username), secure(md5($password)) )) or _error(SQL_ERROR_THROWEN);
        if($get_user->num_rows == 0) {
    ...
    public function socail_register($full_name, $username, $email, $password, $gender, $avatar, $provider) {
        ...
        /* register user */
        $db->query(sprintf("INSERT INTO users (user_name, user_email, user_password, user_fullname, user_gender, user_registered, user_activated, user_picture, $social_id, $social_connected) VALUES (%s, %s, %s, %s, %s, %s, '1', %s, %s, '1')", secure($username), secure($email), secure(md5($password)), secure(ucwords($full_name)), secure($gender), secure($date), secure($image_new_name), secure($_SESSION['social_id']) )) or _error(SQL_ERROR_THROWEN);
        /* get user_id */
        $user_id = $db->insert_id;
        /* affiliates system */
        $this->process_affiliates("registration", $user_id);
